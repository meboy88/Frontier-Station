...
/obj/item/pda
	name = "\improper PDA"
	desc = "A portable microcomputer by RoboCo Industries, distributed by Frontier Inc. Functionality determined by a preprogrammed ROM cartridge."
	...
	var/note = "Congratulations, Frontier Inc has chosen the RobCo 5230 Personal Data Assistant!" //Current note in the notepad function
	var/notehtml = ""
			...
			if ("Toggle_Font")
				...
				switch(font_index)
					...
					if (MODE_VT)
						font_mode = FONT_VT
			if ("Change_Color")
				var/new_color = input("Please enter a color name or hex value (Default is \'#286600\').",background_color)as color
		...
		if(!owner)
			...
			if(((src in user.contents) || (isturf(loc) && in_range(src, user))) && (C in user.contents))
				if(!id_check(user, idcard))
					return
				to_chat(user, "<span class='notice'>You slip the ID into \the [src]'s slot.</span>")
